// Espelha POST /admin/expense-requests/:id/reject (expenses.js:150,
// requireAuth + requireApprover). Propõe rejeitar uma despesa pendente.
...
const definition = {
  type: 'function',
  function: {
    name: 'propor_rejeitar_despesa',
    description: 'Propõe rejeitar uma despesa pendente. Requer confirmação. Use o id que veio da ferramenta de pendências. Não invente id.',
    parameters: {
      type: 'object',
      properties: {
...
async function propose(_profile, args) {
  ...
  return {
    kind: 'rejeitar_despesa',
    payload: { id: row.id, admin_note },
    descricao: `Rejeitar despesa "${row.title}" de ${valor.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} de ${row.pessoa || 'colaborador'}.`,
    dados: {
...
export default {
  kind: 'write',
  espelha: 'POST /admin/expense-requests/:id/reject',
