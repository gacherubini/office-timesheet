// Espelha POST /admin/vacation-requests/:id/reject (vacations.js:344,
...
const definition = {
  type: 'function',
  function: {
    name: 'propor_rejeitar_ferias',
    description: 'Propõe rejeitar uma solicitação de férias pendente. Requer confirmação. Use o id que veio da ferramenta de pendências. Não invente id.',
    parameters: {
      type: 'object',
      properties: {
...
async function propose(profile, args) {
  ...
  return {
    kind: 'rejeitar_ferias',
    payload: { id: row.id, admin_note },
    descricao: `Rejeitar férias de ${row.pessoa || 'colaborador'} de ${inicio} a ${fim} (${row.days_count} dias).`,
    dados: {
...
async function execute(profile, payload) {
  ...
  return { before: { id: rows[0].id, status: 'pending' }, after: rows[0] }
}

export default {
  kind: 'write',
  espelha: 'POST /admin/vacation-requests/:id/reject',
